...
# Search by title to find address (e.g., 0x55a9b1c)
CON_ID=$(hyprctl -j clients | jq -r '.[] | select(.title == "'"$CHEATSHEET_TITLE"'") | .address')

if [ -n "$CON_ID" ]; then
    # Pass 'address:$CON_ID' inside the window parameter table
    hyprctl dispatch "hl.dsp.window.close({ window = 'address:$CON_ID' })"
else
...
-- Helper function to read saved animation state (1 = true, 0 = false)
local function get_animation_state()
    local cache_path = os.getenv("HOME") .. "/.cache/hypr_animations_state"
    local file = io.open(cache_path, "r")
    if not file then return true end -- default to enabled
...
hl.config({
    input = {
        ...
        -- Mouse Acceleration (flat / adaptive)
        accel_profile = "adaptive",
        -- Mouse Sensitivity (-1.0 to 1.0)
        sensitivity = 0.5,
        -- Scroll Speed (0.1 to 4.0)
        scroll_factor = 0.8,
        touchpad = {
            natural_scroll = true,
            tap_to_click = true,
...
hl.bind(mod .. " + " .. "D", hl.dsp.exec_cmd("pidof wayscriber || wayscriber --active"))

-- Edit this hyprland config file (Mod + Shift + Ctrl + h)

hl.bind(mod .. " + " .. "SHIFT + CTRL" .. " + " .. "H", hl.dsp.exec_cmd("alacritty -e nvim ~/.config/hypr/hyprland.lua"))
...
hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "C", hl.dsp.exec_cmd("~/.local/bin/toggle-cheatsheet.sh"))

-- Take a screenshot (Mod + Shift + S)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "S", hl.dsp.exec_cmd("~/.local/bin/screenshot.sh"))

-- Settings for input devices like mouse and touchpad (Mod + Shift + I)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "I", hl.dsp.exec_cmd("~/.local/bin/input-config.sh"))

-- Settings for displays (Mod + Shift + D)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "D", hl.dsp.exec_cmd("~/.local/bin/display-settings.sh"))

-- Wallpaper picker (Mod + Shift + W) (Must have some images in /home/username/Pictures/Wallpapers to select them)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "W", hl.dsp.exec_cmd("~/.local/bin/set-wallpaper.sh"))

-- GTK GUI settings (Mod + Shift + T)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "T", hl.dsp.exec_cmd("nwg-look"))

-- Theme switcher (Mod + T)

hl.bind(mod .. " + " .. "T", hl.dsp.exec_cmd("~/.local/bin/theme-switcher.sh"))

-- Toggle application Launcher (Mod + Space)

hl.bind(mod .. " + " .. "SPACE", hl.dsp.exec_cmd("~/.local/bin/toggle-wofi.sh"))

-- Open power menu (Mod + Shift + Q)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "Q", hl.dsp.exec_cmd("~/.local/bin/power-menu.sh"))

-- Open account manager (Mod + Shift + A)

hl.bind(mod .. " + " .. "SHIFT" .. " + " .. "A", hl.dsp.exec_cmd("~/.local/bin/account-management.sh"))

-- Lock the screen (Mod + Ctrl + Shift + L)

hl.bind(mod .. " + " .. "CTRL + SHIFT" .. " + " .. "L", hl.dsp.exec_cmd("LOCK_WALLPAPER=$(cat " .. os.getenv("HOME") .. "/.cache/lastwallpaper) hyprlock"))

-- Open task manager (Mod + Shift + Esc)

hl.bind("CTRL + SHIFT" .. " + " .. "ESCAPE", hl.dsp.exec_cmd("lxtask"))

-- Reload waybar (top bar) in case of bugs (Mod + Ctrl + Shift + W)

hl.bind(mod .. " + " .. "SHIFT + CTRL" .. " + " .. "W", hl.dsp.exec_cmd("killall waybar && waybar &"))
...
hl.bind(mod .. " + " .. "mouse_up", hl.dsp.exec_cmd("~/.local/bin/dynamic-workspaces.sh next"))

hl.bind(mod .. " + " .. "mouse_down", hl.dsp.exec_cmd("~/.local/bin/dynamic-workspaces.sh prev"))
...
hl.bind("XF86AudioRaiseVolume", hl.dsp.exec_cmd("swayosd-client --output-volume 5 --max-volume 155"), { repeating = true })

-- Lower volume by 5%, maximum volume is 155% (set to any value you want)

hl.bind("XF86AudioLowerVolume", hl.dsp.exec_cmd("swayosd-client --output-volume -5 --max-volume 155"), { repeating = true })
...
hl.bind("XF86MonBrightnessUp", hl.dsp.exec_cmd("swayosd-client --brightness +5 && ~/.local/bin/brightness-control.sh+"), { repeating = true })

-- Lower brightness by 5%

hl.bind("XF86MonBrightnessDown", hl.dsp.exec_cmd("swayosd-client --brightness -5 && ~/.local/bin/brightness-control.sh-"), { repeating = true })
...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("gsettings set io.github.celluloid-player.Celluloid mpv-config-enable true")
    hl.exec_cmd("gsettings set io.github.celluloid-player.Celluloid mpv-config-file \"file://" .. os.getenv("HOME") .. "/.config/mpv/mpv.conf\"")
    hl.exec_cmd("gsettings set io.github.celluloid-player.Celluloid mpv-input-config-enable true")
    hl.exec_cmd("gsettings set io.github.celluloid-player.Celluloid mpv-input-config-file \"file://" .. os.getenv("HOME") .. "/.config/mpv/input.conf\"")
    -- GNOME Software Settings
    hl.exec_cmd("gsettings set org.gnome.software download-updates false")
    hl.exec_cmd("gsettings set org.gnome.software check-interval 7")
